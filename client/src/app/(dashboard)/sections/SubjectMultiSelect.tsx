// client/src/app/(dashboard)/sections/SubjectMultiSelect.tsx
"use client"
...
interface SubjectMultiSelectProps { field: ControllerRenderProps<any, "subjects">; }
        ...
        <div className="flex flex-wrap gap-1">
          {selectedSubjects.map(subject => (
            <Badge key={subject._id} variant="secondary">
              {subject.name}
              <button type="button" className="ml-1 rounded-full outline-none" onClick={() => handleUnselect(subject._id)}>
            ...
            <CommandList className="max-h-[150px] overflow-y-auto">
              <CommandGroup heading="Suggestions">
                {availableSubjects.map(subject => (
                  <CommandItem key={subject._id} onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); }} onSelect={() => handleSelect(subject._id)} className="cursor-pointer">
