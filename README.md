...
- Manage academic resources (classrooms, faculty, subjects, sections)
...
- **Styling**: Tailwind CSS with custom design tokens for consistent theming (light/dark mode support)
...
- Chose Next.js App Router over Pages Router for improved data fetching patterns and nested layouts
...
- **Database Layer**: MongoDB with Mongoose ODM for schema definitions and data validation
...
**Key Design Decisions**:
- Chose MongoDB over SQL for flexible schema evolution as requirements change (e.g., adding equipment to classrooms)
...
2. **Constraint Tracking**: Maintain maps to track faculty schedule, classroom availability, and section timetables
...
4. **Conflict Avoidance**: Check constraints before scheduling:
   ...
   - Classroom must be free and have sufficient capacity
...
- **Export**: Uses ExcelJS to generate `.xlsx` files with current data
...
- Validation occurs before database insertion to maintain data integrity
...
- Frontend stores token in localStorage and includes it in Authorization headers
...
- Distinct values extracted for filtering options (departments, course types)
...
- `JWT_EXPIRE`: Token expiration time (optional, default 7d)
